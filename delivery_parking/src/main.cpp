...
#define WAIT_FOR_delivery_parking_TIME_SEC 10
using namespace std;
...
void state_callback(const std_msgs::String stateMsg)
{
    ...
    if(state.data!="delivery_parking")
        init_var();
}
...
int main(int argc, char** argv){
  ros::init(argc, argv, "delivery_parking");

  ros::NodeHandle n;

  ros::Publisher delivery_parking_state_pub = n.advertise<std_msgs::Int16>("/delivery_parking_state", 1);
...
string filePath;
  ...
  while(n.ok()){
    ...
    if(delivery_parking_state.data==0)
    {
        ...
        if(state_string == "delivery_parking")
        {
            if(timer_check==0)
            {
                timer_check=1;
                timer_time=ros::Time::now();
                ROS_INFO("delivery_parking");
            }

            ROS_INFO("time : %f " ,(current_time-timer_time).toSec()); 
            if((current_time-timer_time).toSec()>WAIT_FOR_delivery_parking_TIME_SEC)
            {
                ROS_INFO("go_delivery_parking");
                delivery_parking_state.data=1;
    ...
    if(delivery_parking_state.data==1)
    {
    ...
    delivery_parking_state_pub.publish(delivery_parking_state);   
