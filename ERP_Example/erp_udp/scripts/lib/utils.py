...
class purePursuit :
    ...
    def steering_angle(self):
        ...
        if self.is_look_forward_point:
            self.steering=atan2((2*self.vehicle_length*sin(theta)),self.lfd)*180/pi * 0.6
                ...
                for path in ref_path.poses :
                    ...
                    if global_vaild_object[i][0]==0 :
                        ...
                        if dis<10.0:
                            ...
                            if rel_distance < min_rel_distance or rel_distance < 7:
                                min_rel_distance=rel_distance
                                self.Person=[True,i]
                                

                    elif global_vaild_object[i][0]==3 and not len(tl)==0 :
                                  
                        
                        if global_vaild_object[i][3] == tl[0] :
                            if tl[1] == 48 or tl[1]==16   :   #
                                traffic_sign ='GO'
                                self.traffic=[False,i]    
