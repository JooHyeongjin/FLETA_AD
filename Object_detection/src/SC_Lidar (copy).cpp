...
pcl::PointCloud <pcl::PointXYZI> LIDAR::Transform(pcl::PointCloud <pcl::PointXYZI> point){
    ...
    pcl::PointCloud <pcl::PointXYZI> filter; double theta = 0.0;
    ...
    for(size_t i = 0; i < cloud->points.size(); ++i){
      ...
      if (inliers->indices.size () == 0)
      {
        ...
        theta = buffer;
          ...
          theta = acos(coefficients->values[2] / sqrt( pow(coefficients->values[0], 2) + pow(coefficients->values[1], 2) + pow(coefficients->values[2], 2)));
      }
     float x = 0.0; float z = 0.0;
     x = point.at(point.size()-1).x - point.at(0).x; z = point.at(point.size()-1).z - point.at(0).z;
     float Pi = 3.14159265359;
     float Radian = Pi / 180;
     if(theta/Radian >= 10 && theta/Radian <= 15) buffer = theta;
     else{
         theta = buffer;
     }
...
pcl::PointCloud <pcl::PointXYZI> LIDAR::ObjectSeg(pcl::PointCloud <pcl::PointXYZI> point){
    ...
    for(int i = 0; i < point.size(); i++){
        if(point.at(i).z >= -0.80 && point.at(i).z <= 1) {
            if((point.at(i).x <= -2.5 || point.at(i).x >= 0) || (point.at(i).y <= -1 || point.at(i).y >= 1)) result.push_back(point.at(i));
        }
    }
