...
std_msgs::Int16 path_decision;
//string path_decision = no;             /////////////////////////
...
ros::Publisher path_decision_pub;
...
void parkingstateCB(const std_msgs::Int16 parkingstateMsg)
{
    ...
    cout << "parking_state :" << parking_state.data << endl;
}
...
int main(int argc, char **argv){
    ...
    path_decision_pub = n.advertise<std_msgs::Int16>("path_decision",1);
...
//        obsatcle_boundary = obsatcle_boundary;
    ...
    ros::Subscriber parking_state_sub = n.subscribe("/parking_state", 1, parkingstateCB);
...
//                int local_index = temp_local_map.info.width*temp_local_map.info.height - (pixel_x*temp_local_map.info.width + pixel_y) -1;
    ...
    //                    cv::ellipse(temp_obstacle_image, cv::Point(pixel_y,pixel_x),cv::Size(obsatcle_boundary,front_obsatcle_boundary),0,0,360,cv::Scalar(95),1,CV_FILLED);
                        if(state_string == "static_obs" || state_string == "big_obs")
                        {
...
//            state_string = "static_obs";
            if(state_string == "static_obs" ||trffic_state_string == "outbreak_obs")
            {
                ...
                for(int i = 1;i<temp_path.poses.size();i++)
                {
                    ...
                    {
                        if((int)temp_map_image.at<unsigned char>(pixel_x,pixel_y) > 60)
                        {
                            if(stuck_count == 0)
                            {
                            ...
                            while(!((int)temp_map_image.at<unsigned char>(pixel_x,pixel_y_left) < 80 || (int)temp_map_image.at<unsigned char>(pixel_x,pixel_y_right) < 80))
                            {
                                ...
                                local_point_y_left = local_point_y_left + 0.1; //0.1
...
//                                }
                            ...
                            // if((int)temp_map_image.at<unsigned char>(pixel_x,pixel_y_right) < 50 || -0.5 < local_point_y_right)
                            // {
                            ...
                            //     //local_point_y = local_point_y_right;
                            // }                            
                            if((int)temp_map_image.at<unsigned char>(pixel_x,pixel_y_left) < 90)
                            {
                                local_point_y = local_point_y_left;
                            }
                            else {
...
//                }
                ...
                ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                // if (front_obsatcle_boundary > 10)
                // {
                //     path_decision.data = 1;
                // }
                // else
                // {
                //     path_decision.data = 2;

                // }
                cout <<"obsatcle_boundary = " << obsatcle_boundary << "\n front_obsatcle_boundary = " << front_obsatcle_boundary << "\n";

                //path_decision_pub.publish(path_decision);
                ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                if(obsatcle_boundary > 20)
                {
                    obsatcle_boundary = 20;   /////// 35
                    under_path = 0;
                }
                if(front_obsatcle_boundary > 25)
                {
                    front_obsatcle_boundary = 25;           /////50
                    under_path = 0;
                    //temp_path = previous_path;
                }
...
//                                }
                            ...
                            if((int)temp_map_image.at<unsigned char>(pixel_x,pixel_y_right) < 90 || -0.8 < local_point_y_right)
                            {
                                local_point_y = local_point_y_right;                                
                                //local_point_y = local_point_y_left;
                            }
...
//                }
                ...
                cout <<"o f" << obsatcle_boundary << ", " << front_obsatcle_boundary << "\n";
                if(obsatcle_boundary > 60)
                {
                    obsatcle_boundary = 60;
                    under_path = 0;
                }
                if(front_obsatcle_boundary > 65)
                {
                    front_obsatcle_boundary = 65;
                    under_path = 0;
                }
//                if(obsatcle_boundary > 30 && front_obsatcle_boundary > 60)
//                {
...
//                temp_path = emptyPath;
            ...
            //if(parking_state.data = 0 && state_string != "parking_search")
            if (state_string != "parking_search" || parking_state.data != 1)
            {
                ...
                else if (state_string != "parking_search"  && parking_state.data == 0) 
                {
                    if (state_string != "parking" && parking_state.data == 0)
                    {
                        local_path_pub.publish(temp_path);
                        temp_local_map.data = temp_data;
                        local_costmap.publish(temp_local_map);
