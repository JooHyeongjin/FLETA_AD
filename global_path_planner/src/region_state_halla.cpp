...
const string state_table[11] = {"go","stop","big_obs","slow_down_for_downhill","static_obs","outbreak_obs","parking_search","parking","delivery_parking","school_zone","finish"};
/////////////////////////////////255 //20 /40///////60///////////////////////////80////////////100//120//140///////160/////////////////180/////200
                                                                                                                    //6             //7
...
void hallaobsCallback(const std_msgs::String::ConstPtr& msg)
{
  halla_obs = msg->data.c_str();
  ROS_INFO("halla : %s", halla_obs.c_str());
}
...
int main(int argc, char **argv){
    ...
    ros::Subscriber halla_obs = n.subscribe("/halla_obs", 1, hallaobsCallback);
...
//                cout << point << "\n";
                ...
                else
                {
                    ...
                    cout << "halla obs state : " << halla_obs << "\n";
                    cout << "only left? : " << trf_point << "\n";
                    if(state_string == "go" && halla_obs == "find_person")
                    {
                        final_state = "stop";
                        cout << "stop find person!\n";
                    }
                    if(state_string == "slow_down_for_downhill")
                    {
                        final_state = "slow_down_for_downhill";
                    }
                    if(state_string == "slow_down_for_downhill" && halla_obs == "find_person")
                    {
                        final_state = "stop";
                        cout << "stop find person!\n";
                    }
                    else if(state_string == "slow_down_for_downhill" && (halla_obs == "GREEN" || halla_obs == "LEFT"))
                    {
                        final_state = "go";
                        if(trf_point < 200)
                        {
                    ...
                    if(state_string == "delivery_parking") ////////////////////////////// mddddd
                    {
                        final_state = "delivery_parking";
                        cout << "delivery_parking\n";
                    }
