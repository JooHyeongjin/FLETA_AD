...
void globalpathCallback(const nav_msgs::Path::ConstPtr& msg)          /////////////////// 글로벌 맵 불러오는거 
{
    if(!global_loaded)
    {
...
void obstacleCallback(const sensor_msgs::PointCloud2ConstPtr& scan) //////////////////////////////////////////////////////////
{
    ...
    // if (obstacle_pcl_save +200 < obstacle_pcl)
    // {
    ...
    // }
    // obstacle_pcl_save = obstacle_pcl ;
}                                                                   ////////////////////////////////////////////////////////
...
int main(int argc, char **argv){
    ros::init(argc, argv, "local_path_made");
    ros::NodeHandle n;
...
//        obsatcle_boundary = obsatcle_boundary;
    ...
    while (ros::ok())
    {
        // if(!map_loaded)          /////////////////////52번줄 확인시 처음 반복시 무조건 조건을 만족한다
        // {
...
//            }
//        }
        if(pose_count != 0)          /////////////////////GPS 데이터 받으면 무조건 작동한다
        {
            if(state_ok)
            {
                state_ok = false;
            ...
            temp_local_map.info.resolution = 0.1;
            temp_local_map.info.origin.position.y = -(int)(size_side*0.1);
            ...
            double region_map_x = (current_x + 182.4)/resolution;
            double region_map_y = (current_y + 626)/resolution;
...
;
                    ...
                    unsigned int map_x = (global_x + 182.4)/resolution;
                    unsigned int map_y = (global_y + 626)/resolution;                    
                    //int map_index = map_y*plain_map.info.width + map_x;
                    int map_index = map_y*3306 + map_x;
                    ...
                    //cout <<"plain_map.info.width" << plain_map.info.width << "\n";

                    if(map_index >= 10764*3306)
                    {
                    ...
                    else
                    {
                        ...
                        //original_map_data = plain_map.data.at(map_index);            ////////////////////////// 이거 처리?

                        original_map_data = 0;
                        //cout <<"test5 : " << original_map_data << "\n";
...
//                int local_index = temp_local_map.info.width*temp_local_map.info.height - (pixel_x*temp_local_map.info.width + pixel_y) -1;
    ...
    //                        plain_map.data.at(map_index) = plain_map.data.at(map_index)+10;
                        ...
                        //el``lipse(Mat& img, Point center, Size axes, double angle, double startAngle, double endAngle, const Scalar& color, int thickness=1, int lineType=8, int shift=0)
                    }
                }

            }
//            obstacle_pcl.clear();         /////////////////////////// 라이더 인식 끝 ////////////////////////////////////
            ...
            bool is_stuck = false;
            int path_index = path_index_start;          ////////////////////// 글로벌 패스의 크기 만큼
            while(far_euclidean < max_global_path && path_index < global_path->poses.size())            /////////////// 조건을 만족하면 계속 반복한다.
            {
...
//                path_index_start++;
            ...
            for(int i = 0;i<closest_index;i++)          //////////////// temp_path의 크기만큼 반복
            {
...
//            state_string = "static_obs";
            ...
            if(state_string == "static_obs"  || trffic_state_string == "outbreak_obs")
            {
                ...
                for(int i = 1;i<temp_path.poses.size();i++)
                {
                    ...
                    if(pixel_y < temp_map_image.cols && pixel_x < temp_map_image.rows && pixel_x>0&&pixel_y>0)     ///////////temp_map_image=temp_obstacle_image
                    {
...
//                                }
                            ...
                            cout << "local_point_y_left" << local_point_y_left << endl;
                            cout << "local_point_y_right" << local_point_y_right << endl;


                            //if((int)temp_map_image.at<unsigned char>(pixel_x,pixel_y_left) < 93)|| pixel_y_left  < pixel_y_right - 60 || abs(local_point_y_left) < abs(local_point_y_right)
                            if((int)temp_map_image.at<unsigned char>(pixel_x,pixel_y_right) < 94 || -0.8 < local_point_y_right)
                            {
...
//                }
                ...
                if(obsatcle_boundary > 15)
                {
                    obsatcle_boundary = 15;
                    under_path = 0;
                }
...
//                    under_path = 0;
//                }
                if(obsatcle_boundary < 20)
                {
                    obsatcle_boundary = 20;
//                    under_path = 0;
                }
                if(front_obsatcle_boundary < 30)
                {
                    front_obsatcle_boundary = 30;
//                    under_path = 0;
                }
...
//            system("clear");         //////////////////////////////////////////publish
            if(stuck_count > 0)
            {
                ...
                local_obstacle_state.publish(obs_state);                  /////////////// .'자리에 publish 오타??
            }
            else {
                cout << "\n \n obstacle free!\n\n";
                cout << arg_str;
